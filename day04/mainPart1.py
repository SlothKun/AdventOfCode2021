...
with open("input.txt", 'r') as file:
    ...
    while not win:
        chosenNb = numbers[index]
        for board in boards:
            for lineNb in range(len(board["board"])):
                for colNb in range(len(board["board"][lineNb])):
                    # Check for winning board only if number is found
                    if board["board"][lineNb][colNb] == chosenNb:
    ...
    # print final score
    print("Final score : ", unmarkedSum * int(numbers[index-1]))
