...
class Probe:
    ...
    def checkCoordinates(self):
        if self.targetCoor[0][0] <= self.currentCoor[0] <= self.targetCoor[0][1] and \
            ...
            self.inTarget = True
        elif self.currentCoor[0] > self.targetCoor[0][1] or self.currentCoor[1] < self.targetCoor[1][1]:
    ...
    def launchProbes(self):
        ...
        while x < MAX_X: # While it hasn't attained the point of non touching
            ...
            while y < MAX_Y:
                ...
                if probe.move():
                    if probe.getHighestY() > self.probeBestHeight:
                        self.probeBestHeight = probe.getHighestY()
                y += 1
        print("answer : ", self.probeBestHeight)
